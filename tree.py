...
"""while max_depth <= 100:
    ...
    tfidf.fit([i['text'] for i in raw_train + test_links])
...
m_params = {}
m_params['splitter'] = "best"
m_params['max_depth'] = 90
m_params['criterion'] = 'entropy' #gini = 0.519 log_loss = 0.534
m_params['max_features'] = 'sqrt'  # int, float or sqrt auto
m_params['max_leaf_nodes'] = None
...
print(classification_report([i for i in sub_pred], [i['sentiment'] for i in test_links],
                            target_names=['negative', 'neutral', 'positive'], digits=5))
